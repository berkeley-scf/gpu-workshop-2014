...
// CUDA kernel:
__global__ void calc_loglik(float* vals, int N, float mu, float sigma) {
...
int calc_loglik_cpu(float* vals, int N, float mu, float sigma) {
  float std, e;
...
int main (int argc, char *argv[]) {
  ...
  for( N = 32768; N <= 134217728; N*=8 ) {
    cpu_vals = (float*) malloc( sizeof(float)*N );
    cudaStat = cudaMalloc(&gpu_vals, sizeof(float)*N);
    ...
    cudaStat = cudaMemcpy(gpu_vals, cpu_vals, N*sizeof(float), cudaMemcpyHostToDevice);
